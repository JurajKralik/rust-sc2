...
impl Bot {
	...
	*/
	...
	/// Gets pre-calculated tactical positions for siege units (e.g., Siege Tanks).
	/// 
	/// These positions are evaluated for:
	/// - High ground advantage
	/// - Minimal walkable approaches (better defense)
	/// - Coverage of multiple choke points
	/// - Sufficient space for larger units
	/// 
	/// Positions are sorted by tactical value (best first).
	/// 
	/// # Returns
	/// Vector of tactical positions, or None if pathfinding not initialized.
	pub fn get_tactical_positions(&self) -> Option<&Vec<sc2pathfinding::TacticalPosition>> {
		self.pathfinding_map.as_ref().map(|map| map.tactical_positions())
	}

	/// Gets tactical positions with automatic pathfinding initialization.
	/// 
	/// This version automatically initializes pathfinding if it hasn't been set up yet.
	/// See `get_tactical_positions()` for details on what makes a position tactical.
	/// 
	/// # Returns
	/// Vector of tactical positions sorted by score (best first).
	pub fn get_tactical_positions_lazy(&mut self) -> &Vec<sc2pathfinding::TacticalPosition> {
		self.ensure_pathfinding_initialized();
		self.pathfinding_map.as_ref().unwrap().tactical_positions()
	}

	/// Calculate custom tactical positions for a specific unit type.
	/// 
	/// This allows you to calculate positions optimized for units with different
	/// sizes and ranges than the default Siege Tank configuration.
	/// 
	/// # Parameters
	/// - `unit_radius`: Radius of the unit in game units (e.g., 1.25 for Siege Tank)
	/// - `attack_range`: Attack range of the unit (e.g., 13.0 for Siege Tank)
	/// 
	/// # Returns
	/// Vector of tactical positions sorted by score (best first), or None if pathfinding not initialized.
	pub fn calculate_tactical_positions(&self, unit_radius: f32, attack_range: f32) 
		-> Option<Vec<sc2pathfinding::TacticalPosition>> {
		self.pathfinding_map.as_ref().map(|map| {
			map.calculate_tactical_positions(unit_radius, attack_range)
		})
	}

	/// Find the best tactical positions near a specific location.
	/// 
	/// Filters pre-calculated tactical positions to only those within a certain
	/// distance of the target position, sorted by tactical score.
	/// 
	/// # Parameters
	/// - `position`: Center point to search around
	/// - `max_distance`: Maximum distance from the position
	/// 
	/// # Returns
	/// Vector of tactical positions within range, sorted by score (best first).
	pub fn get_tactical_positions_near(&self, position: Point2, max_distance: f32) 
		-> Option<Vec<sc2pathfinding::TacticalPosition>> {
		self.pathfinding_map.as_ref().map(|map| {
			let positions = map.tactical_positions();
			let mut nearby_positions = Vec::new();
			
			for pos in positions {
				let distance = position.distance(pos.position);
				if distance <= max_distance {
					nearby_positions.push(pos.clone());
				}
			}
			
			// Already sorted by score from tactical calculation
			nearby_positions
		})
	}
