	...
	Self::Item: Borrow<Unit>,
{
	...
	fn find_tags<'a, T: Container<u64>>(self, tags: &'a T) -> FindTags<'a, Self, T> {
	...
	fn of_types<'a, T: Container<UnitTypeId>>(self, types: &'a T) -> OfTypes<'a, Self, T> {
		OfTypes::new(self, types)
	}
	/// Excludes units of given types.
	fn exclude_types<'a, T: Container<UnitTypeId>>(self, types: &'a T) -> ExcludeTypes<'a, Self, T> {
