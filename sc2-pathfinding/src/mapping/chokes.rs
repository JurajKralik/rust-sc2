...
pub struct Choke {
    ...
    pub side1: Vec<(f32, f32)>,
    pub side2: Vec<(f32, f32)>,
    pub pixels: Vec<(usize, usize)>,
    pub min_length: f32,
}
impl Choke {
    pub fn lines(&self) -> &Vec<((f32, f32), (f32, f32))> { &self.lines }
    pub fn side1(&self) -> &Vec<(f32, f32)> { &self.side1 }
    pub fn side2(&self) -> &Vec<(f32, f32)> { &self.side2 }
    ...
    pub fn center(&self) -> (f32, f32) {
        ((self.main_line.0.0 + self.main_line.1.0) / 2.0,
         (self.main_line.0.1 + self.main_line.1.1) / 2.0)
    }
}

impl Choke {
    pub fn new(line: ((usize, usize), (usize, usize))) -> Self {
        let p0 = (line.0.0 as f32, line.0.1 as f32);
        let p1 = (line.1.0 as f32, line.1.1 as f32);
        let lines = vec![(p0, p1)];
        let side1 = vec![p0];
        let side2 = vec![p1];
    ...
    fn add_line(&mut self, point1: (usize, usize), point2: (usize, usize)) {
        let p1 = (point1.0 as f32, point1.1 as f32);
        let p2 = (point2.0 as f32, point2.1 as f32);
    ...
    fn remove_excess_lines(&mut self) {
        ...
        for line in &self.lines {
            let d = distance((line.0.0 as usize, line.0.1 as usize), (line.1.0 as usize, line.1.1 as usize));
            distances.push(d);
            if d < min_distance {
                min_distance = d;
            }
    ...
    fn set_points(&mut self, points: &mut Vec<Vec<map_point::MapPoint>>) {
        for line in &self.lines {
            let pos1 = Pos(line.0.0 as usize, line.0.1 as usize);
            let pos2 = Pos(line.1.0 as usize, line.1.1 as usize);
    ...
    fn calc_final_line(&mut self) {
        ...
        let point1 = (x_sum / self.side1.len() as f32, y_sum / self.side1.len() as f32);
        ...
        let point2 = (x_sum / self.side2.len() as f32, y_sum / self.side2.len() as f32);
